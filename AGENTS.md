...
## 🚨🚨🚨 ABSOLUTE REQUIREMENT: Read Docs Before ANY Code 🚨🚨🚨
...
### ❌ YOU ARE NOT ALLOWED TO GENERATE CODE WITHOUT READING THE DOCS FIRST ❌

**No exceptions. No shortcuts. No assumptions.**

Before generating **ANY** code — including snippets, examples, stubs, or full implementations — you **MUST** use the `read_file` tool (or equivalent) to **fully read** every documentation file in the `/docs` directory that is relevant to your task.

This means:

1. **STOP** — before writing any code, identify which `/docs` files apply to your task
2. **READ** — use `read_file` to open and read each relevant file in full
3. **APPLY** — follow every pattern, constraint, and guideline found in those files
4. **ONLY THEN** — generate code that conforms to what you just read

---

### Why This Is Non-Negotiable

Skipping this step **will** cause:
...
- Wasted time fixing entirely preventable mistakes

There is **no acceptable reason** to skip reading the docs. Even if you believe you already know the pattern, **read the file anyway**. The docs are the source of truth.

---

### Required Documentation by Topic

You **MUST** physically open and read these files using your file-reading tools — do not rely on memory or assumptions:

| Task Area | File to Read | When Required |
|-----------|-------------|---------------|
| Authentication, route protection, user data, auth features | [docs/authentication.md](docs/authentication.md) | **ALWAYS — before any auth-related code** |
| UI components, forms, buttons, styled elements, installing component libraries | [docs/ui-components.md](docs/ui-components.md) | **ALWAYS — before any UI-related code** |

> **If a new `/docs` file exists that is relevant to your task, read it too.**
> When in doubt, read all files in `/docs`.
...
```
1. Receive task
2. Identify ALL relevant /docs files
3. READ each file fully using read_file (or equivalent tool)
4. Confirm you have read and understood the guidelines
5. Generate code that strictly follows those guidelines
```

**If you skip steps 2–4, you are violating the project's core requirement.**
